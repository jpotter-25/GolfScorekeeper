  ...
  app.patch('/api/rooms/:code/settings', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (room.hostId !== userId) {
        return res.status(403).json({ message: "Only the host can update room settings" });
      }
      
      // Check if room is still in pre-game state
      if (room.status !== 'room') {
      ...
      // Validate maxPlayers
      if (maxPlayers && maxPlayers < currentPlayerCount) {
        return res.status(400).json({ 
          message: `Cannot reduce max players below current player count (${currentPlayerCount})` 
        });
      }
      ...
      if (stakeBracket !== undefined) {
        updates.stakeBracket = stakeBracket;
      }
      
      // Increment version for tracking
      const currentVersion = (room as any).version || 0;
      updates.version = currentVersion + 1;
      
      // Update room
      const updatedRoom = await storage.updateGameRoom(code, updates);
      
      if (!updatedRoom) {
        return res.status(500).json({ message: "Failed to update room" });
      }
      
      console.log(`Room ${code} settings updated by host (version: ${updates.version})`);
      
      // Broadcast updated room to all subscribers
      const broadcastFn = (global as any).broadcastRoomUpdate;
      if (broadcastFn) {
        await broadcastFn('updated', updatedRoom);
      }
      
      // Also broadcast to specific room subscribers
      const roomBroadcastFn = (global as any).broadcastRoomSnapshot;
      if (roomBroadcastFn) {
        await roomBroadcastFn(code, updatedRoom);
      }
